...
Trade: ${trade}
City: ${city}
${finalUrl ? `Website: ${finalUrl}` : "Website: (none)"}
...
Search the web now, then return the scan JSON.`;
    ...
    try {
      scan = await runWebGroundedJSON({ system: SYSTEM_PROMPT, user, maxTokens: 4096, maxSearches: 5, signal });
    } catch (e) {
      await failAudit(auditId);
      throw new Error("Scan failed or timed out — try again.");
    }
